package Graph;

public class NodeStatus {

	public final static String UNKNOWN = "UNKNOWN";
	public final static String PATCHED = "PATCHED";
	public final static String VULNERABLE = "VULNERABLE";
	public final static String HACKED = "HACKED";

	public final static String ACTION_PATCH = "HACKED";
	public final static String ACTION_HACK = "HACKED";
	public final static String ACTION_SCAN = "HACKED";

	public final static String VAR_NODES = "NODES";
	public final static String VAR_EDGES = "EDGES";
	public final static String VAR_LIST = "LIST";
	public final static String VAR_START = "START";
	public final static String VAR_END = "END";
}
